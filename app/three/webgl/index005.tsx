...
//创建圆形顶点数据
function createCircle(x: number, y: number, redius: number, n: number) {
  let points = [x, y, 1, 0, 0]; // 初始化点数组，包含圆心坐标和颜色
  let a1 = [];
  for (let index = 0; index < n; index++) {
    let [r, g, b] = [Math.random(), Math.random(), Math.random()];
    const angle = (index / n) * 2 * Math.PI; // 计算当前点的角度
    const pointX = x + redius * Math.cos(angle); // 计算当前点的 x 坐标
    const pointY = y + redius * Math.sin(angle); // 计算当前点的 y 坐标
    if (index === 0) a1.push(pointX, pointY, r, g, b);
    points.push(pointX, pointY, r, g, b); // 将当前点添加到点数组中
  }
  return [...points, ...a1];
}

//创建圆环顶点数据
function createCirquePoints(x: number, y: number, outerRedius: number, innerRedius: number, n: number) {
  ...
  for (let i = 0; i < n; i++) {
    let [r, g, b] = [Math.random(), Math.random(), Math.random()];
    color = { r, g, b };
    const angle = (i / n) * 2 * Math.PI; // 计算当前点的角度
    const innerX = x + innerRedius * Math.cos(angle); // 计算内圆当前点的 x 坐标
    const innerY = y + innerRedius * Math.sin(angle); // 计算内圆当前点的 y 坐标
    const outerX = x + outerRedius * Math.cos(angle); // 计算外圆当前点的 x 坐标
    const outerY = y + outerRedius * Math.sin(angle); // 计算外圆当前点的 y 坐标
    positon.push(innerX, innerY, color.r, color.g, color.b); // 将当前点添加到点数组中
    positon.push(outerX, outerY, color.r, color.g, color.b); // 将当前点添加到点数组中
    let v0 = i * 2; // 内圆点的索引
    let v1 = v0 + 1; // 外圆点的索引
    let v2 = v0 + 2; // 下一个内圆点的索引
    let v3 = v0 + 3; // 下一个外圆点的索引
    if (i == n - 1) {
      v2 = 0; // 如果是最后一个点，连接到第一个点
      v3 = 1; // 如果是最后一个点，连接到第二个点
    }
    // indexs.push(v0, v1, v2, v2, v1, v3); // 添加第一个三角形的索引【三角扇绘制环形的顶点数据】
    indexs.push(v0, v1, v2, v3);
  }
  return { positon, indexs };
}
...
function drawTriangle(gl: WebGLRenderingContext, program: WebGLProgram) {
  ...
  const pontsPoston = createCirquePoints(100, 100, 100, 50, 50).positon; // 创建圆环顶点数据
  const pontsIndex = createCirquePoints(100, 100, 100, 50, 50).indexs; // 创建圆环索引数据

  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(pontsPoston), gl.STATIC_DRAW);
  gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(pontsIndex), gl.STATIC_DRAW); // 定义圆环的索引
  ...
  gl.clear(gl.COLOR_BUFFER_BIT); // 清除颜色缓冲区
  // gl.drawElements(gl.TRIANGLES, pontsIndex.length, gl.UNSIGNED_SHORT, 0); // 三角扇绘制圆环
  gl.drawElements(gl.TRIANGLE_STRIP, pontsIndex.length, gl.UNSIGNED_SHORT, 0); // 使用三角带绘制圆环
}

export function Gl005() {
  useEffect(() => {
